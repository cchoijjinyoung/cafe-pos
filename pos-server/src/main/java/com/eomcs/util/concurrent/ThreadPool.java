      ...
      synchronized (this) {
        while (true) {
          ...
          try {
            ...
            System.out.printf("[%s] - %s", this.getName(), e.getMessage());
            // 작업 수행중 오류가 발생하더라도
            // 스레드는 계속 유효하다.
          } finally {
            // 정상적인 종료든 작업 수행중 예외가 발생했든 간에
            // 작업을 마친 스레드는 재사용할 수 있도록 다시
            // 목록에 보관되어야 한다.
            workers.add(this);
    ...
    try {
      ...
      while (!workers.isEmpty()) { // 스레드풀에 대기중인 스레드가 있다면,
        ...
        synchronized (worker) {
          ...
          // 스레드는 깨어나면 stopping 상태에 따라 종료여부를 결정한다.
          // => Worker 스레드 코드를 보라!
